...
export default class RowManager extends InstancePlugin {
    ...
    getRecordCoordsByIndex(recordIndex, local = false) {
        const
            ...
            height                = me.heightMap.get(id) || me.averageRowHeight || (me._rowHeight + me.grid._rowBorderHeight),
    ...
    /**
     * Store supplied `height` using `id` as key in the height map, which used to calculate better averages/tops.
     * Ignored when configured with `fixedRowHeight`
    ...
    /**
     * Invalidate cached height for a record
    ...
    calculateTop(index) {
        ...
        const { heightMap, rowOffsetHeight, store } = this;
        ...
        for (let i = 0; i < index; i++) {
            const { id } = store.getAt(i);
            // Use known height when available
            if (heightMap.has(id)) {
                top += heightMap.get(id);
            }
    ...
    /**
     * Calculates `averageRowHeight` based on known heights in the height map
     * @private
     */
    updateAverageRowHeight() {
        const me = this;

        // When using fixed row height, there is nothing to update. The truth is known
        if (!me.fixedRowHeight) {
            let totalHeight = 0;

            me.heightMap.forEach(height => totalHeight += height);

            me.totalKnownHeight = totalHeight;
            me.averageRowHeight = totalHeight / me.heightMap.size;
        }
    }

    /**
     * Resets `averageRowHeight`
     * @param {Boolean} fullReset
     */
    resetAverageRowHeight(fullReset = false) {
        const me = this;

        if (!me.fixedRowHeight) {
            me.averageRowHeight = null;
    ...
    estimateTotalHeight() {
        ...
        if (me.fixedRowHeight) {
            ...
            estimate = me.totalKnownHeight + Math.floor(unknownCount * me.rowOffsetHeight);
