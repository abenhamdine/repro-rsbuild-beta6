...
export default class HorizontalEventMapper extends BaseHorizontalMapper {
    ...
    onTimeAxisViewModelUpdate() {
        ...
        // Anything might happen to events... all bets on row height are off
        this.scheduler.rowManager.clearKnownHeights();
    ...
    onEventAdd({ records, resources }) {
        ...
        records.forEach(event => {
            ...
            if (startDate && endDate && me.timeAxis.timeSpanInAxis(startDate, endDate)) {
                ...
                resources.forEach(resource => me.cache.clearRow(resource.id, false, true));

                // Also invalidate cached row height, as it might no longer be correct
                me.scheduler.rowManager.invalidateKnownHeight(resources);
            }
        });
    }
    ...
    onEventRemove({ resources }) {
        ...
        // Also invalidate cached row height, as it might no longer be correct
        this.scheduler.rowManager.invalidateKnownHeight(resources);
    }
    ...
    onEventUpdate({ source : eventStore, record : eventRecord, changes = {}, resources }) {
        ...
        if (resourceWasChanged) {
            ...
            rowManager.invalidateKnownHeight([oldResource, newResource]);
        ...
        // start or end date changed, need to update layout
            ...
            rowManager.invalidateKnownHeight(resources);
        }
